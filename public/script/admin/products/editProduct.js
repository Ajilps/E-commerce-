...
// const id = document.getAttribute("proId").value;
...
const imageDeleteButtons = document.querySelectorAll(".productImage");
imageDeleteButtons.forEach((button) => {
  button.addEventListener("click", function (e) {
    e.preventDefault();
    const imageIndex = String(this.getAttribute("imgIndex"));
    // console.log(imageIndex)
    deletedImageIndexes.push(imageIndex);
    deleteProductImage(this);
  });
});
const deleteProductImage = async (button) => {
  Swal.fire({
    title: "Are you sure?",
    ...
    if (result.isConfirmed) {
      // removing the image container from the form
      const imageContainer = button.parentNode;
      console.log(`image button ${imageContainer.parentNode.remove()}`);
...
// sending a get request for fetching the tags of the product
window.onload = async function (e) {
  const tagFromDb = await fetch(`getTags/${productId}`)
...
function addTag(tag) {
  const tagText = tag || tagInput.value.trim();
...
form.addEventListener("submit", async (e) => {
  ...
  calAndUpdate(); //calling price calculate on submit
  ...
  try {
    ...
    formData.append("sizes", sizeInput);
    ...
    // appendDeletedImageIndexToFormData(formData)
    formData.append("deleteImageIndexes", JSON.stringify(deletedImageIndexes));
    //testing
    console.log(`image index from form data  ${formData.deleteImageIndexes}`);
    const response = await fetch(`/admin/products/editProducts/${productId}`, {
      method: "POST",
      body: formData,
    });
