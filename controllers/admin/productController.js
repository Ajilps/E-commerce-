...
const displayAddProducts = async (req, res) => {
  ...
  res.status(200).render("admin/products/addProduct.ejs", {
    brand,
    category,
    user: req.user,
  });
...
const listProducts = async (req, res) => {
  ...
  let filter = req?.query?.filter || "all"; // Default filter is "all"
  const limit = 3;

  // Base match query for search
  let matchQuery = {
  ...
  switch (filter) {
    case "Available":
      matchQuery.status = "Available";
      break;
    case "lowStock":
      matchQuery.quantity = { $lt: 10 };
      break;
    case "Discontinued":
      matchQuery.status = "Discontinued";
      break;
    case "outOfStock":
  ...
  const products = await Product.aggregate([
    ...
    .skip((page - 1) * limit)
    .limit(limit)
    .allowDiskUse(true); // Allow the db to use disk

  // Get the total count of products matching the search and filter
...
const displayEditProduct = async (req, res) => {
  ...
  res.status(200).render("admin/products/editProduct.ejs", {
    product,
    brand,
    category,
    user: req.user,
...
const displayAddVariant = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).render("admin/products/addVariant.ejs", {
      parentProduct,
      brand,
      category,
      user: req.user,
...
const saveVariantProduct = async (req, res) => {
  try {
    ...
    if (!parentId)
      res
        .status(400)
        .json({ success: false, message: "Parent product id is required " });
